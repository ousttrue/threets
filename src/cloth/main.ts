...
onload = function() {
  // GL関係のインスタンスを生成
  const gl = new yrGL("canvas_main");
  const renderer = gl.createRenderer();									// レンダラ―
  const material_constant = gl.createMaterial(vs_constant, fs_constant);	// マテリアル

  // タイマー
  const timer = new yrTimer();

  // 入力
  const input = new yrInput(gl._canvas);
  ...
  // 布（後で初期化する）
  let cloth = undefined;
  const scale = 1.0;	// 布の大きさに対するスケーリング（ベースのサイズは2*2）

  // その他、フレームを跨ぐパラメータ
  let ms_acc = 0;		// 累積時間
  let ms_surplus = 0;	// 更新処理の余剰時間（次フレームに持ち越す分）
