...
// 更新（エディタモード）
yrCamera.prototype.updateEditorMode = function(d_rot_x, d_rot_y, d_length) {
...
// 更新（FPSモード）
yrCamera.prototype.updateFPSMode = function(d_rot_x, d_rot_y, d_forward, d_backward, d_left, d_right) {
  this._up = vec3.fromValues(0.0, 1.0, 0.0);	// 固定
...
// ビューマトリクス取得
yrCamera.prototype.getViewMatrix = function() {
  var m = mat4.create();
...
// プロジェクションマトリクス取得
yrCamera.prototype.getProjectionMatrix = function(is_perspective) {
  var m = mat4.create();
  if (is_perspective) {
